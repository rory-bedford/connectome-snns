...
def assign_weights_lognormal(
    ...
    Args:
        ...
        cell_type_signs (IntArray | None): Signs for each SOURCE cell type (+1/-1) as a numpy array.
            This is used to determine if connections are excitatory or inhibitory based on Dale's law.
            Length must equal the number of unique source cell types.
            If None, weights will be unsigned (conductance-based model).
    ...
    # Validate cell_type_signs
    if cell_type_signs is not None:
        assert isinstance(cell_type_signs, np.ndarray), (
            "cell_type_signs must be a numpy array (IntArray) or None."
        )
        n_source_types = len(cell_type_signs)
    else:
        # Infer number of source types from source_cell_indices
        n_source_types = max(source_cell_indices) + 1
    ...
    # Apply signs if provided (Dale's law), otherwise return unsigned weights
    if cell_type_signs is not None:
    ...
    for src_type in range(n_source_types):
        for tgt_type in range(n_target_types):
            ...
            # Get the specified parameters for this cell type combination
            specified_mu = w_mu_matrix[src_type, tgt_type]
            specified_sigma = w_sigma_matrix[src_type, tgt_type]
            ...
            if len(nonzero_weights) > 0:
                ...
                print(
                    ...
                    f"(specified: mu={specified_mu:.4f}, sigma={specified_sigma:.4f})"
                )
            else:
                print(
