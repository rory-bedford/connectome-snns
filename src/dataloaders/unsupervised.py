...
# Named tuples for different dataloader outputs
InhomogeneousPoissonData = namedtuple("InhomogeneousPoissonData", ["input_spikes"])
InhomogeneousPoissonDataWithRates = namedtuple(
    "InhomogeneousPoissonDataWithRates", ["input_spikes", "rates", "weights"]
)
HomogeneousPoissonData = namedtuple(
    "HomogeneousPoissonData", ["input_spikes", "pattern_idx"]
)
...
class HomogeneousPoissonSpikeDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> HomogeneousPoissonData:
        """
        ...
        Returns:
            HomogeneousPoissonData named tuple with fields:
                - input_spikes: torch.Tensor of shape (n_steps, n_neurons)
        ...
        return HomogeneousPoissonData(input_spikes=spikes, pattern_idx=pattern_idx)
...
def collate_pattern_batches(batch):
    ...
    Args:
        batch: List of HomogeneousPoissonData named tuples from HomogeneousPoissonDataset.
               Each input_spikes tensor has shape (n_steps, n_neurons).
               Length of batch = batch_size * n_patterns

    Returns:
        HomogeneousPoissonData named tuple with fields:
            - input_spikes: torch.Tensor of shape (batch_size, n_patterns, n_steps, n_neurons)
            - pattern_idx: List of unique pattern indices

    Example:
        >>> # DataLoader with batch_size=5, dataset has 10 patterns
        >>> # DataLoader fetches 50 items total (5 repeats × 10 patterns)
        >>> # batch = [HomogeneousPoissonData(spikes0, 0), ..., HomogeneousPoissonData(spikes9, 9),  # First repeat
        >>> #          HomogeneousPoissonData(spikes0, 0), ..., HomogeneousPoissonData(spikes9, 9),  # Second repeat
        >>> #          ...]  # 5 repeats total
        >>> # Each input_spikes has shape (n_steps, n_neurons)
        >>> #
        >>> # Result after collation:
        >>> # result.input_spikes.shape = (5, 10, n_steps, n_neurons)
        >>> #                              ↑  ↑   ↑       ↑
        >>> #                              │  │   │       └─ neurons
        >>> #                              │  │   └───────── time steps
        >>> #                              │  └───────────── patterns
    ...
    for item in batch:
        spikes_list.append(item.input_spikes)  # Each is (n_steps, n_neurons)
        pattern_indices.append(item.pattern_idx)
    ...
    return HomogeneousPoissonData(
        input_spikes=batched_spikes, pattern_idx=unique_patterns
    )
...
def _collate_single_pattern(batch):
    ...
    Args:
        batch: List of HomogeneousPoissonData named tuples.
               Each input_spikes tensor has shape (n_steps, n_neurons).

    Returns:
        HomogeneousPoissonData named tuple with fields:
            - input_spikes: torch.Tensor of shape (batch_size, n_steps, n_neurons)
            - pattern_idx: List containing single pattern index [0]
    """
    spikes_list = []
    for item in batch:
        spikes_list.append(item.input_spikes)  # Each is (n_steps, n_neurons)
    ...
    return HomogeneousPoissonData(input_spikes=batched_spikes, pattern_idx=[0])
...
class InhomogeneousPoissonSpikeDataset(Dataset):
    ...
    def __getitem__(
        self, idx: int
    ) -> Union[InhomogeneousPoissonData, InhomogeneousPoissonDataWithRates]:
        """
        ...
        Returns:
            If return_rates=False:
                InhomogeneousPoissonData named tuple with field:
                - input_spikes: shape (chunk_size, n_neurons) - binary spike tensor
            If return_rates=True:
                InhomogeneousPoissonDataWithRates named tuple with fields:
                - input_spikes: shape (chunk_size, n_neurons) - binary spike tensor
        ...
        if self.return_rates:
            return InhomogeneousPoissonDataWithRates(
                input_spikes=spikes, rates=rates, weights=weights
            )
        else:
            return InhomogeneousPoissonData(input_spikes=spikes)
        ...
        while True:
            if self.return_rates:
                ...
                for _ in range(self.batch_size):
                    ...
                    spikes_batch.append(data.input_spikes)
                    rates_batch.append(data.rates)
                    weights_batch.append(data.weights)
                    idx += 1
                ...
                yield InhomogeneousPoissonDataWithRates(
                    input_spikes=spikes_tensor,
                    rates=rates_tensor,
                ...
                for _ in range(self.batch_size):
                    data = self.dataset[idx]
                    batch.append(data.input_spikes)
                    idx += 1

                # Stack into (batch_size, chunk_size, n_neurons)
                batch_tensor = torch.stack(batch, dim=0)
                yield InhomogeneousPoissonData(input_spikes=batch_tensor)
