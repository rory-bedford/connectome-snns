...
from training_utils.checkpointing import load_checkpoint
from configs import (
...
def main(
    ...
    (
        ...
        variance=weight_perturbation_variance,
    ...
    model = ConductanceLIFNetwork(
        ...
        batch_size=batch_size,  # From teacher dataset
        weights_FF=feedforward_weights,
        cell_type_indices_FF=feedforward_cell_type_indices,
        cell_params_FF=feedforward.get_cell_params(),
        synapse_params_FF=feedforward.get_synapse_params(),
        scaling_factors=np.array(scaling_factors["recurrent"]),
        scaling_factors_FF=np.array(scaling_factors["feedforward"]),
        optimisable=optimisable,
    ...
    def stats_computer(spikes, model_snapshot):
        ...
        for cell_type in np.unique(cell_type_indices):
            ...
            cell_type_name = recurrent.cell_types.names[int(cell_type)]
        ...
        # Get cell type names
        target_cell_types = recurrent.cell_types.names
        source_ff_cell_types = feedforward.cell_types.names
    ...
    if resume_from is None:
        ...
        print(f"\nRunning inference with initial weights ({plot_size} chunks)...")
        inference_duration_ms = plot_size * chunk_size * spike_dataset.dt
    ...
    print(
        f"Log interval: {log_interval} chunks ({log_interval * chunk_duration_s:.1f}s)"
    )
    print(
        f"Checkpoint interval: {checkpoint_interval} chunks ({checkpoint_interval * chunk_duration_s:.1f}s)"
    )

    # Reset model state back to training batch size (may have been changed for inference)
    model.reset_state(batch_size=batch_size)
    model.track_variables = False  # Ensure tracking is disabled for training start
    model.use_tqdm = False  # Disable tqdm for training
