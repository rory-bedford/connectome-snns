"""
Training visible neurons with feedforward dynamics - NO PERTURBATION TEST.

This is a test variant of train_feedforward_hidden_units.py where the student
starts with scaling factors that already perfectly match the teacher (no perturbation).
This tests whether the trained state is a stable minimum or if parameters drift.

Hidden neurons are:
- Removed from the recurrent weight matrix (both as sources and targets)
- Excluded from the input spike trains (teacher activity)
- Not used as outputs for training
"""
...
def make_hidden_collate_fn(
    ...
    def hidden_collate_fn(batch: SpikeData) -> SpikeData:
        ...
        # Use float32 since hidden neurons get fractional rates, not binary spikes
        ...
        return SpikeData(
            ...
            target_spikes=visible_target_spikes,
        )

    return hidden_collate_fn
...
def main(
    ...
    """Train visible neurons to match target spike train activity using feedforward dynamics.

    Hidden neurons are randomly selected and excluded from inputs, outputs, and connectome.
    ...
    # Load hidden cell fraction (not in config class, read directly)
    hidden_cell_fraction = data["simulation"].get("hidden_cell_fraction", 0.0)
    ...
    if n_hidden > 0:
        # Randomly select neurons to hide
    ...
    spike_dataloader = DataLoader(
        ...
        num_workers=0,
        collate_fn=hidden_collate_fn,
    )

    # ==============================================
    # Initialize Feedforward Network (Visible Neurons)
    # ==============================================
    ...
    def plot_generator(
        ...
        **kwargs,
        ...
        # target_spikes: (batch, time, n_visible) - target spikes
        ...
        fig = plot_spike_trains(
            ...
            fraction=1.0,
            random_seed=None,
            title=f"Feedforward Network: Target vs Trained (first {n_plot} visible neurons, {hidden_cell_fraction:.0%} hidden)",
            ylabel="Neuron",
            figsize=(14, 8),
        )
    ...
    def stats_computer(spikes, model_snapshot):
        ...
        spikes_np = spikes[0, :, :]  # (time, n_visible)

        # Compute mean firing rate across all visible neurons
        spike_counts = spikes_np.sum(axis=0)  # (n_visible,)
    ...
    print(
        f"Training {n_neurons} visible neurons with Van Rossum loss ({n_hidden} hidden)"
    )
    ...
    np.savez(
        ...
        # Also save mapping back to full network
        visible_indices=visible_indices,
        hidden_indices=hidden_indices,
        n_neurons_full=n_neurons_full,
    )
