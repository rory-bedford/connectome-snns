...
2. M-step (Training): Train the feedforward network to match ALL unit activity (visible from teacher, hidden from inference)

Key difference from train_hidden_units_em.py: Loss is computed on ALL neurons (visible + hidden),
not just visible neurons. Hidden targets come from the E-step inference.
"""
...
def make_em_collate_fn(
    ...
    - Targets: ALL neurons (visible from teacher + hidden from inference)
    ...
    def em_collate_fn(batch: SpikeData) -> SpikeData:
        ...
        # Target: ALL neurons (visible from teacher + hidden from inferred)
        ...
        # Visible targets from teacher
        full_targets[:, :, visible_tensor] = batch.target_spikes[
            :, :, visible_tensor
        ].float()
        # Hidden targets from inferred spikes
        full_targets[:, :, hidden_tensor] = hidden_spikes_chunk

        return SpikeData(
            input_spikes=concatenated_inputs,
            target_spikes=full_targets,
        )
...
def main(
    ...
    """Train hidden units using EM-like algorithm with loss on ALL units.

    E-step: Run recurrent network to infer hidden unit activities
    M-step: Train feedforward network on ALL units (visible from teacher, hidden from inference)
    ...
    # ===============================================
    # Weights for Feedforward Model (ALL Outputs)
    # ===============================================
    # Unlike the visible-only version, we keep weights to ALL neurons

    concatenated_weights_full_perturbed = np.concatenate(
        [perturbed_ff_weights, perturbed_rec_weights], axis=0
    )

    concatenated_mask_full = np.concatenate(
        [feedforward_mask, recurrent_mask],
        axis=0,
    )

    print("\n✓ Feedforward network setup (all inputs -> ALL outputs):")
    print(f"  - Output neurons: {n_neurons_full} (ALL neurons)")
    print(
        f"  - Total inputs per neuron: {n_total_inputs} ({n_feedforward} FF + {n_neurons_full} recurrent)"
    )
    print(f"  - Weight matrix shape: {concatenated_weights_full_perturbed.shape}")
    ...
    # ==============================================
    # Initialize FEEDFORWARD Model (ALL Outputs)
    # ==============================================

    print("\n✓ Initializing feedforward model for training (M-step, ALL outputs)...")

    feedforward_model = FeedforwardConductanceLIFNetwork(
        dt=dt,
        weights_FF=concatenated_weights_full_perturbed,  # ALL neurons as outputs
        cell_type_indices=cell_type_indices,  # ALL neuron types
        ...
        feedforward_mask=concatenated_mask_full,  # Full mask
        track_variables=False,
        use_tqdm=False,
    )
    ...
    def make_plot_generator(inferred_spikes_zarr_path: Path, chunk_counter_ref: list):
        ...
        def plot_generator(
            ...
            **kwargs,
            ...
            # spikes now has ALL neurons, so we need to index into visible
            n_plot = min(10, n_visible)

            # Randomly select visible neurons to plot
            plot_indices_visible = np.random.choice(
                n_visible, size=n_plot, replace=False
            )
            # Map to full network indices
            full_indices_visible = visible_indices[plot_indices_visible]

            interleaved = np.zeros((1, spikes.shape[1], 2 * n_plot))
            for i, (local_idx, full_idx) in enumerate(
                zip(plot_indices_visible, full_indices_visible)
            ):
                interleaved[0, :, 2 * i] = target_spikes[0, :, full_idx]
                interleaved[0, :, 2 * i + 1] = spikes[0, :, full_idx]
            ...
            fig_visible = plot_spike_trains(
                ...
                fraction=1.0,
                random_seed=None,
                title=f"EM Training (Full Loss): Target vs Trained (first {n_plot} visible neurons, {hidden_cell_fraction:.0%} hidden)",
                ylabel="Neuron",
                figsize=(14, 8),
            )
            ...
            if n_hidden > 0:
                ...
                # Get inferred hidden spikes from zarr (these are the targets for hidden neurons)
                ...
                teacher_hidden = teacher_all_plot[:, hidden_indices][np.newaxis, :, :]
                ...
                fig_hidden = plot_spike_trains(
                    ...
                    fraction=1.0,
                    random_seed=None,
                    title=f"EM Training (Full Loss): Teacher vs Inferred (first {n_plot_hidden} hidden neurons, {hidden_cell_fraction:.0%} hidden)",
                    ylabel="Neuron",
                    figsize=(14, 8),
                )
    ...
    def make_stats_computer(inferred_spikes_zarr_path: Path, chunk_counter_ref: list):
        ...
        Args:
            inferred_spikes_zarr_path: Path to zarr file with inferred spikes (unused in full loss version)
            chunk_counter_ref: Reference to current chunk counter for indexing
        """
        # Note: In the full loss version, student spikes already contain all neurons
        # (visible + hidden) from the model output, so we don't need to read inferred
        # spikes from zarr here - they're already in the model output.
        ...
        def stats_computer(spikes, model_snapshot):
            ...
            # Student: from model output (spikes accumulated over multiple chunks)
            # Now spikes has ALL neurons
            student_all = spikes[0, :, :]  # (time_accumulated, n_neurons_full)
            n_timesteps_accumulated = student_all.shape[0]

            duration_s = n_timesteps_accumulated * dt / 1000.0

            # Split into visible and hidden
            student_visible = student_all[:, visible_indices]
            student_hidden = student_all[:, hidden_indices]
            ...
            teacher_all = np.array(teacher_spikes_zarr[:1, start_t:end_t, :])[0]

            teacher_visible = teacher_all[:, visible_indices]
            teacher_hidden = teacher_all[:, hidden_indices]
    ...
    print("\n" + "=" * 60)
    print("Starting EM training (FULL LOSS on all neurons):")
    ...
    print(f"  - Loss computed on: ALL {n_neurons_full} neurons (visible + hidden)")
    print("=" * 60)
        ...
        wandb_config_dict = {
            **wandb_config,
            "config": {
                ...
                "device": device,
                "loss_mode": "full",  # Indicate this is the full loss version
            },
        }
    ...
    for em_iter in range(total_iterations):
        ...
        # ===== M-STEP: TRAINING =====
        print(
            f"\n--- M-Step: Training feedforward network for {epochs_per_update} epochs (FULL LOSS) ---"
        )
        ...
        trainer = SNNTrainer(
            ...
            feedforward_mask=torch.from_numpy(
                concatenated_mask_full.astype(np.float32)
            ).to(device),
    ...
    np.savez(
        ...
        feedforward_connectivity=concatenated_mask_full,
        cell_type_indices=cell_type_indices,  # ALL neuron types
