...
from configs import SimulationConfig, TrainingConfig, Hyperparameters, Targets
...
def main(
    ...
    trainer = SNNTrainer(
        ...
        device=device,
        num_epochs=simulation.num_chunks,
    ...
    log_interval_s = training.log_interval * simulation.chunk_duration_s
    checkpoint_interval_s = training.checkpoint_interval * simulation.chunk_duration_s
    print(f"Log interval: {training.log_interval} chunks ({log_interval_s:.1f}s)")
    print(
        f"Checkpoint interval: {training.checkpoint_interval} chunks ({checkpoint_interval_s:.1f}s)"
    )
    ...
    for source_idx in range(len(recurrent.cell_types.names)):
        source_mask = cell_type_indices == source_idx
        for target_idx in range(len(recurrent.cell_types.names)):
    ...
    for source_idx in range(len(feedforward.cell_types.names)):
