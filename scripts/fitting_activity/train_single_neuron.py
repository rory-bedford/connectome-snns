"""
Training a single neuron with feedforward dynamics to match target activity.

This script trains a single neuron (neuron 0) from a connectome-constrained network
using feedforward-only dynamics. It concatenates recurrent and feedforward neurons as
inputs to create a unified feedforward architecture for training a single output neuron.

This is a simplified version of train_student.py designed for testing and debugging
single-neuron learning dynamics.
"""
...
def main(
    ...
    """Train a single neuron to match target spike train activity.
    ...
    # ==============================================
    # Extract Single Neuron (Neuron 0) Connectivity
    # ==============================================

    # Extract neuron 0's cell type
    neuron_0_cell_type = cell_type_indices[0:1]  # Shape: (1,)

    # Extract row 0 from recurrent weights and column 0 from feedforward weights
    # Recurrent: inputs from all recurrent neurons to neuron 0
    recurrent_to_neuron0 = weights[0, :]  # Shape: (n_neurons,)
    recurrent_mask_to_neuron0 = recurrent_mask[0, :]  # Shape: (n_neurons,)

    # Feedforward: inputs from all feedforward neurons to neuron 0
    feedforward_to_neuron0 = feedforward_weights[:, 0]  # Shape: (n_feedforward,)
    feedforward_mask_to_neuron0 = feedforward_mask[:, 0]  # Shape: (n_feedforward,)

    # Concatenate feedforward then recurrent (order matters for indices!)
    # Shape: (n_feedforward + n_neurons, 1)
    concatenated_weights = np.concatenate(
        [feedforward_to_neuron0, recurrent_to_neuron0]
    )[:, np.newaxis]

    concatenated_mask = np.concatenate(
        [feedforward_mask_to_neuron0, recurrent_mask_to_neuron0]
    )[:, np.newaxis]

    n_feedforward = len(feedforward_cell_type_indices)
    n_recurrent = len(cell_type_indices)
    n_total_inputs = n_feedforward + n_recurrent

    print("\n✓ Single neuron setup:")
    print(f"  - Neuron 0 cell type: {neuron_0_cell_type[0]}")
    print(
        f"  - Total inputs: {n_total_inputs} ({n_feedforward} FF + {n_recurrent} recurrent)"
    )
    ...
    sf_feedforward = np.array(params.scaling_factors["feedforward"])
    sf_recurrent = np.array(params.scaling_factors["recurrent"])
    ...
    sigma = np.sqrt(training.weight_perturbation_variance)
    mu = -(sigma**2) / 2.0
    ...
    for input_idx in range(n_total_inputs):
        ...
        output_type = neuron_0_cell_type[0]
        perturbed_weights[input_idx, 0] *= perturbation_factors[input_type, output_type]
    ...
    # ==============================================
    # Initialize Single-Neuron Feedforward Network
    # ==============================================

    model = FeedforwardConductanceLIFNetwork(
        dt=spike_dataset.dt,
        weights_FF=perturbed_weights,
        cell_type_indices=neuron_0_cell_type,
    ...
    print("\n✓ Model initialized:")
    print(f"  - Single output neuron (type {neuron_0_cell_type[0]})")
    print(f"  - {n_total_inputs} feedforward inputs")
    ...
    def plot_generator(
        ...
        **kwargs,
        ...
        """Generate spike train comparison plot."""
        # spikes: (1, time, 1) - trained single neuron
        # target_spikes: (1, time, 1) - target single neuron

        # Interleave target and trained for comparison
        # Create (1, time, 2) array with [target, trained]
        interleaved = np.concatenate([target_spikes, spikes], axis=2)

        # Create cell type indices for coloring
        cell_type_indices_plot = np.array([0, 1])  # 0=target, 1=trained
        ...
        fig = plot_spike_trains(
            ...
            fraction=1.0,
            random_seed=None,
            title="Single Neuron: Target vs Trained",
            ylabel="Neuron",
            figsize=(14, 4),
        )
    ...
    def stats_computer(spikes, model_snapshot):
        """Compute summary statistics for single neuron."""
        # spikes shape: (1, time, 1)
        spikes = spikes[0, :, 0]  # (time,)

        # Compute firing rate
        spike_count = spikes.sum()
        duration_s = len(spikes) * spike_dataset.dt / 1000.0
        firing_rate = float(spike_count / duration_s)

        stats = {
            "firing_rate/neuron_0": firing_rate,
        }
        ...
        input_cell_type_names = (
            params.feedforward.cell_types.names + params.recurrent.cell_types.names
        )
        output_cell_type_names = params.recurrent.cell_types.names
    ...
    # The trainer reads from params.feedforward.cell_types.names for labeling
    # Since we concatenated recurrent types as additional inputs, update the config
    params.feedforward.cell_types.names = (
        params.feedforward.cell_types.names + params.recurrent.cell_types.names
    )
