...
def main():
    ...
    with tracker:
        ...
        cbar.ax.set_yticklabels(
            ["Inhibitory (-1)", "No connection (0)", "Excitatory (+1)"]
        )
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / "04_mitral_cell_spikes.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / "06_dp_network_spikes.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
        ...
        spike_counts = (
            output_spikes[0].sum(axis=0).cpu().numpy()
